...
func (c *Client) Ping() (ping types.Ping, e error) {
	_, e = c.Path("ping").Receive(&ping)
	return
}
...
func (c *Client) Version() (version types.Version, e error) {
	_, e = c.Path("version").Receive(&version)
	return
}
...
func (c *Client) Login(user, pass string) (token types.LoginToken, e error) {
	_, e = c.Path("login").Post(types.Login{Email: types.EmailAddress(user), Password: types.NonEmptyString(pass)}).Receive(&token)
	return
}
...
func (c *Client) RefreshToken() (token types.LoginToken, e error) {
	_, e = c.Path("refresh_token").Post("").Receive(&token)
	return
}
